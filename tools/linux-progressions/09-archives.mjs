...
export default {
  ...
  source: [
    {
      title: 'Load several exported settings', focus: 'source applies every assignment in a reviewed environment file to the current shell.',
      example: 'source app.env && printf "%s %s\\n" "$APP" "$PORT"', example_output: 'api 8080',
      task: 'Source service.env and print SERVICE followed by PORT.',
      solution: 'source service.env && printf "%s %s\\n" "$SERVICE" "$PORT"', files: { 'service.env': 'SERVICE=worker\nPORT=9000\n' }, checks: out('worker 9000', 'Both sourced settings were available in the current shell.'),
    },
    {
      title: 'Load and call a shell function', focus: 'Functions declared by a sourced file remain callable because the file runs in the current shell.',
      example: 'source helpers.sh && greet Ada', example_output: 'hello Ada',
      task: 'Source lib/format.sh and call label ready.',
      solution: 'source lib/format.sh && label ready', files: { 'lib/format.sh': 'label() { printf "status=%s\\n" "$1"; }\n' }, checks: out('status=ready', 'The sourced function produced the requested label.'),
    },
    {
      title: 'Select a configuration file dynamically', focus: 'A variable can choose which reviewed file source loads, while quoting keeps its path intact.',
      example: 'profile=dev; source "profiles/$profile.env"; printf "%s\\n" "$URL"', example_output: 'http://dev.local',
      task: 'Set profile to test, source profiles/$profile.env, and print ENDPOINT.',
      solution: 'profile=test; source "profiles/$profile.env"; printf "%s\\n" "$ENDPOINT"', files: { 'profiles/test.env': 'ENDPOINT=https://test.invalid\n', 'profiles/prod.env': 'ENDPOINT=https://prod.invalid\n' }, checks: out('https://test.invalid', 'The dynamically selected test endpoint was printed.'),
    },
    {
      title: 'Compose a reusable deployment check', focus: 'A sourced library can provide variables and functions that a short workflow combines with exit status and redirection.',
      example: 'source deploy.sh && validate "$TARGET" > result.txt', example_output: '',
      task: 'Source lib/release.sh, run validate_release "$VERSION", save its output in result.txt, and print result.txt.',
      solution: 'source lib/release.sh && validate_release "$VERSION" > result.txt && cat result.txt', files: { 'lib/release.sh': 'VERSION=2.4.1\nvalidate_release() { [[ $1 =~ ^[0-9]+\\.[0-9]+\\.[0-9]+$ ]] && printf "release=%s valid\\n" "$1"; }\n' }, checks: [content('result.txt', 'release=2.4.1 valid\n'), ...out('release=2.4.1 valid', 'The sourced release workflow wrote and printed a valid result.')],
    },
  ],
  bzip2: [
    {
  ...
  find: [
    {
      title: 'Limit a search by file type', focus: '-type distinguishes regular files, directories, and links before other predicates are applied.',
      example: 'find project -type d', example_output: 'project\nproject/src',
      task: 'Print only directories below workspace in sorted order.',
      solution: 'find workspace -type d | sort', directories: ['workspace/app/lib', 'workspace/docs'], files: { 'workspace/app/main.sh': '', 'workspace/docs/readme.md': '' }, checks: out('workspace\nworkspace/app\nworkspace/app/lib\nworkspace/docs', 'Only directories were printed in deterministic order.'),
    },
    {
      title: 'Combine predicates safely', focus: 'find combines predicates with an implicit AND; quoting a wildcard leaves matching to find.',
      example: "find logs -type f -name '*.log'", example_output: 'logs/api.log',
      task: 'Print only .sh files below project, sorted.',
      solution: "find project -type f -name '*.sh' | sort", files: { 'project/bin/start.sh': '', 'project/lib/check.sh': '', 'project/readme.md': '', 'project/fake.sh/notes.txt': '' }, checks: out('project/bin/start.sh\nproject/lib/check.sh', 'Only regular shell-script files were selected.'),
    },
    {
      title: 'Filter by size and print portable names', focus: '-size selects by file size and -printf can emit only the fields a pipeline needs.',
      example: 'find data -type f -size +1k -printf "%f\\n"', example_output: 'large.bin',
      task: 'Print the basenames of regular files larger than 20 bytes below data, sorted.',
      solution: 'find data -type f -size +20c -printf "%f\\n" | sort', files: { 'data/tiny.txt': 'small\n', 'data/archive/large.txt': 'this file is definitely larger than twenty bytes\n', 'data/medium.txt': '123456789012345678901\n' }, checks: out('large.txt\nmedium.txt', 'Only basenames of files over the threshold were printed.'),
    },
    {
      title: 'Act on exact null-delimited paths', focus: '-print0 and xargs -0 preserve spaces and other ordinary filename characters during batch workflows.',
      example: "find inbox -type f -name '*.tmp' -print0 | xargs -0 rm", example_output: '',
      task: 'Remove every .tmp file below cache, including one with a space, then print each remaining path followed by its changing fixture content.',
      solution: "find cache -type f -name '*.tmp' -print0 | xargs -0 rm --; find cache -type f -print0 | sort -z | xargs -0 awk '{print FILENAME \":\" $0}'", files: { 'cache/old.tmp': '', 'cache/old copy.tmp': '', 'cache/keep.txt': 'keep={{nonce}}\n', 'cache/nested/data.json': 'token={{nonce}}\n' }, checks: [missing('cache/old.tmp'), missing('cache/old copy.tmp'), ...out('cache/keep.txt:keep={{nonce}}\ncache/nested/data.json:token={{nonce}}', 'Temporary files were removed and the retained dynamic records were reported.')],
    },
  ],
  xz: [
    {
  ...
  jq: [
    {
      title: 'Select nested JSON data', focus: 'Chained property filters navigate nested objects without relying on whitespace or key order.',
      example: "jq -r '.server.host' config.json", example_output: 'localhost',
      task: 'Print the nested database host from config.json.',
      solution: "jq -r '.database.host' config.json", files: { 'config.json': '{"database":{"host":"db.internal","port":5432},"debug":false}\n' }, checks: out('db.internal', 'The nested host value was printed as plain text.'),
    },
    {
      title: 'Filter an array of objects', focus: '.[] iterates array elements and select keeps objects whose fields satisfy a condition.',
      example: "jq -r '.[] | select(.active) | .name' users.json", example_output: 'Ada',
      task: 'Print the names of enabled services from services.json, one per line.',
      solution: "jq -r '.[] | select(.enabled) | .name' services.json", files: { 'services.json': '[{"name":"api","enabled":true},{"name":"legacy","enabled":false},{"name":"worker","enabled":true}]\n' }, checks: out('api\nworker', 'Only enabled service names were printed.'),
    },
    {
      title: 'Build a new JSON shape', focus: 'Object construction creates predictable output containing only selected or calculated fields.',
      example: "jq '{name: .service, healthy: (.status == \"ok\")}' status.json", example_output: '{\n  "name": "api",\n  "healthy": true\n}',
      task: 'Transform app.json into a compact object with name and a ports count.',
      solution: "jq -c '{name: .name, port_count: (.ports | length)}' app.json", files: { 'app.json': '{"name":"gateway","ports":[80,443],"internal":"secret"}\n' }, checks: out('{"name":"gateway","port_count":2}', 'The compact JSON object contains only the derived public fields.'),
    },
    {
      title: 'Combine JSON documents in a workflow', focus: 'jq -s reads multiple documents into an array so a filter can aggregate their structured values.',
      example: "jq -s 'map(.count) | add' reports/*.json", example_output: '12',
      task: 'Read every JSON file in reports and print total=<sum of requests>.',
      solution: "printf 'total=%s\\n' \"$(jq -s 'map(.requests) | add' reports/*.json)\"", files: { 'reports/api.json': '{"requests":12}\n', 'reports/web.json': '{"requests":23}\n', 'reports/worker.json': '{"requests":5}\n' }, checks: out('total=40', 'The request counts from all JSON reports were aggregated.'),
    },
  ],
  zip: [
    {
