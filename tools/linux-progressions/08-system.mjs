...
export default {
  ...
  hostname: [
    ...
    {
      title: 'Print the fully qualified host name', focus: 'hostname -f resolves the local host entry and prints its canonical fully qualified name.',
      example: 'hostname -f', example_output: 'training-box.example.test',
      task: 'Print the fully qualified hostname.', solution: 'hostname -f',
      setup: ["short=$(hostname -s); grep -qF \"$short.example.test\" /etc/hosts || printf '127.0.1.1 %s.example.test %s\\n' \"$short\" \"$short\" >> /etc/hosts"],
      checks: [{ type: 'stdout-contains', description: 'The training domain was included.', expected: '.example.test' }],
    },
    {
      title: 'Resolve the host address', focus: 'hostname -i resolves the current host name to one or more network addresses.',
      example: 'hostname -i', example_output: '172.17.0.2',
      task: 'Print the address associated with the current hostname.', solution: 'hostname -i',
      setup: ["short=$(hostname -s); grep -qF \"$short.example.test\" /etc/hosts || printf '127.0.1.1 %s.example.test %s\\n' \"$short\" \"$short\" >> /etc/hosts"],
      checks: out('127.0.1.1'),
    },
    {
      title: 'Create a host identity record', focus: 'Combining hostname queries produces a small record suitable for logs and diagnostics.',
      example: "printf 'host=%s address=%s\\n' \"$(hostname -s)\" \"$(hostname -i)\"", example_output: 'host=training-box address=172.17.0.2',
      task: 'Print host=<short-name> address=<resolved-address>.',
      solution: "printf 'host=%s address=%s\\n' \"$(hostname -s)\" \"$(hostname -i)\"",
      setup: ["short=$(hostname -s); grep -qF \"$short.example.test\" /etc/hosts || printf '127.0.1.1 %s.example.test %s\\n' \"$short\" \"$short\" >> /etc/hosts"],
      checks: [{ type: 'stdout-contains', description: 'The host label was included.', expected: 'host=' }, { type: 'stdout-contains', description: 'The resolved address was labeled.', expected: ' address=127.0.1.1' }],
    },
  ],
  uptime: [
    {
